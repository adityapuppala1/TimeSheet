...
| Model | Purpose | Notes |
|---|---|---|
| `GlobalFaceVerificationSettings` | Singleton (`id = "global"`), same shape as the other settings singletons. | Master switch, scope (`ALL` vs `SELECTED`), calibrated thresholds, retention window, consent wording. |
...
field" shape as `User.hourlyRate` + `GlobalTicketSettings.enableCostAnalytics`.
