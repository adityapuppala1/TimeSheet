...
| Email templates (subject/body per event) | **Email templates** page (sidebar) | Edit any of the 32 built-in templates, preview with sample data, send a single test, or "Send all templates as test" to smoke-test every one at once. Also shows per-template send volume, the success/failure split, and a grouped failure breakdown read from `EmailLog`. |
