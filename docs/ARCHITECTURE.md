...
- **Control plane** (`apps/api/prisma/control/schema.prisma`): `Organization`, `OrgDatabase`
  ...
  `PlanTierLimit` (which since 5.0.0 also carries the operator-editable **list price** per seat, the
  only price anywhere in this product's data), and `OrgUsageSnapshot` — one aggregate row per
...
**Since 5.0.0 the console has its own authority model** (§ API.md, *Governance*). Five roles over
five capabilities, in `packages/shared` beside the tenant `permissions` object but never mixed with
it — they are two authority models over two planes, and a role nobody holds is what you get by
adding them together. Three properties matter architecturally:

- **The role is read from `PlatformAdminUser` on every request**, in `middleware/platform-admin-auth.ts`,
  never carried as a JWT claim. The token holds `sub` and `sid` only. A demotion therefore binds on
  the next request rather than at token expiry, and an unrecognised stored value fails closed to
  `READ_ONLY`.
- **The MFA challenge sits between the password check and `establishSession`**, so a challenged
  login has produced no session row and no refresh cookie. There is nothing to steal at that point,
  because nothing has been minted.
- **Two-person actions replay through the same handler.** `twoPerson(action, execute)` registers the
  executor and returns the queueing route handler from one closure, and approval looks the executor
  up *by action*, never by the stored route string. Every guard therefore re-runs against live data
  at approval time — the alternative, re-deriving what to do from a serialised request, is how a
  countersigned action ends up bypassing a check that was added after it was queued.
...
| `/api/devops/:orgSlug/findings`, `/api/devops/:orgSlug/test-runs`, `/api/devops/:orgSlug/quality-gate` | Same "external caller, no subdomain" reasoning as the chat webhooks — a CI job POSTing security-scan findings or test results, SonarQube's own quality-gate webhook, or a SonarQube/ESLint/SARIF/SBOM payload on one of the sibling `/findings/*` routes. Authenticity is a single shared bearer token per org (`IngestionSettings.encryptedToken`, `crypto.timingSafeEqual`), not a per-vendor signature scheme, since arbitrary SAST/DAST tools don't share one. Rate-limited (120/min) since it's a public POST endpoint doing DB writes. | `controllers/devops-webhook.controller.ts` |
...
| `POST /api/contact` | The public sales enquiry form. A prospect has no workspace, so there is no subdomain to resolve and nothing tenant-scoped to write — the lead lands in the **control plane**, like a signup does. Unauthenticated by design and therefore rate-limited on its own mount (5/hour/IP), with a honeypot and a monotonic-clock fill-time floor rather than a third-party captcha, because the FAQ beside the form promises the app never calls out. | `controllers/sales-lead.controller.ts` |
...
| `components/AiRefine.tsx` (web) | The per-field "refine this text" affordance — a hook plus a trigger and a result panel, so each caller keeps its own layout. Always a **proposal**: original and suggestion side by side, nothing changes until "Use this", and the replaced value is kept so Undo is real. Replaced two older "Improve with AI" buttons that overwrote what the author had typed. | `services/api.ts`, `lib/safe-html.ts` | Timesheet form (description, notes), ticket title/description, ticket comments, every written section of the Weekly AI/ML Practice Update |
| `services/practice-update.service.ts` | The **counted** half of the Weekly AI/ML Practice Update: the period's metrics, the previous period's, and one row per active project with its category, owner and RAG status. Category and owner are inferred (from the project's name, then from what its people logged against it); the RAG is arithmetic over overdue and SLA counts and is **never** asked of a model — a red a model chose is not reproducible in the meeting where somebody asks why. Grouped queries throughout, not a loop over projects. | `prisma`, `change.service.ts` | `practice-update.controller.ts`, `workers/practice-update.worker.ts` |
| `services/practice-update-mail.service.ts` | Turns that data plus an optional narrative into the ten sections of the email. Separate from `mail-templates.ts` because those templates are rendered **twice** — once with real values and once with every argument replaced by `"{{name}}"` for the admin editor — and anything that does arithmetic or iterates an array throws on the second pass. Every narrative section falls back to the facts it would have been written from, per section, so a missing model blanks nothing. | `mail-templates.ts`, `practice-update.service.ts` | `practice-update.controller.ts`, `workers/practice-update.worker.ts` |
| `workers/practice-update.worker.ts` | The optional Monday 07:30 send. Three gates must all be open (the cadence flag, the email category, a non-empty distribution list) and the AI gate is deliberately **not** one of them — the figures send whether or not a model answers. Idempotent by re-reading its own `EmailLog` rows rather than keeping state. | `run-for-every-org.ts`, `practice-update-mail.service.ts`, `mail.service.ts` | `server.ts` |
| `controllers/practice-update.controller.ts` | Draft / review / send, plus the distribution list. `requireAuth, requireSuperAdmin` on the **whole router**: it aggregates every project, everyone's hours and every open finding into one document and mails it to arbitrary addresses. `/send` rebuilds the figures server-side and takes only the prose from the request — a caller who could post numbers into a document that looks like this workspace's records could make it say anything. | `practice-update.service.ts`, `ai.service.ts`, `mail.service.ts` | `app.ts` |
...
| `utils/finding-fingerprint.ts` | The stable identity of a finding: tool + rule identity (CWE, else `ruleId`) + normalised path + the line **bucketed** in fifties, sha256'd behind a `v1:` version prefix so the recipe can change without silently re-deduplicating history against a different rule. Returns `null` rather than guessing when a finding carries no path or no rule identity — ingest then falls back to create-always, which is honest about what it does not know. | `node:crypto` | `devops-webhook.controller.ts`, `security-report.service.ts` |
| `services/security-report.service.ts` (5.0.0 additions) | `markFindingsAwaitingVerification` on resolve/close, `verifyFindingsAgainstScanRun` on every ingest, `sweepUnverifiedFindings` nightly, and `resolveReopenDigestAudience`. The qualifying-scan predicate is deliberately narrow — same tool, repository, branch **and type** — and the tool comparison is done in JS rather than SQL because MySQL collation would make a case-insensitive match a property of the column rather than of the rule. | `finding-fingerprint.ts`, `notify.service.ts` | `ticket.controller.ts`, `devops-webhook.controller.ts`, `verification-sweep.worker.ts` |
| `workers/verification-sweep.worker.ts` | Daily at 04:50 UTC, per org. Marks claims that no qualifying scan reached inside the window as `UNVERIFIED` and nudges — and reopens nothing, ever. The one worker in this codebase whose job is to record an absence of evidence rather than to act on evidence. | `run-for-every-org.ts`, `security-report.service.ts` | `server.ts` |
| `utils/path-pattern.ts` | The glob matcher for module path rules, simulated as an automaton over a boolean array of reachable positions rather than compiled to a RegExp. Backtracking blowup is impossible rather than mitigated, which is the correct posture for a pattern an admin types and a webhook exercises unattended. `compilePathPattern` returns `null` and never throws. | — | `finding-routing.service.ts`, `finding-routing.controller.ts` |
| `services/finding-routing.service.ts` | Repository pattern → project, then path glob → module/submodule, both ordered and first-match-wins — the semantics `TicketRule` already uses, so this codebase has one rule-evaluation model. `resolveFindingLocation` is what both the ingest path and the "Test a path" dry-run call, so the preview cannot drift from the behaviour. | `path-pattern.ts`, `config/prisma.ts` | `security-report.service.ts`, `finding-routing.controller.ts` |
...
### Shared document & date primitives (api)

Two modules that exist because the same thing had been written several times, and the copies had
drifted. Both are the "one definition" answer to a class of bug rather than a new capability.

| File | Purpose | Depends on | Depended on by |
|---|---|---|---|
| `services/pdf-kit.ts` | The house style for every PDF the product produces: palette, watermark, running header, footers, measured tables with repeating headers, status pills, bullet lists, a small markdown renderer, and a chart renderer that draws bar/line/pie with PDFKit primitives. A **factory** rather than bare functions, because the four documents genuinely disagree about geometry — the requirements document is inset to balance its cover and contents pages, the three operational reports are wider because they are dense tables and 24pt is a whole column. What is deliberately NOT here is each document's own structure: cover pages, section order, and what a report's totals mean stay with the document. | `pdfkit` (types only) | `requirements-doc-pdf.service.ts`, `timesheet-report-pdf.service.ts`, `attestation-pdf.service.ts`, `security-report-pdf.service.ts` |
| `utils/date-window.ts` | The `from`/`to` window the home page's date filter drives, parsed and resolved once: `parseDayWindow`, `workDateFilter` (date columns), `resolveTimestampWindow` (timestamp columns, plus the equal-length period before it), `windowDays`. Three of its callers had grown their own copy of the same eight-line parser, and the off-by-one in an inclusive range — an exclusive end must sit at midnight on the day AFTER `to`, or the window silently drops its own last day — is exactly the kind of thing that should exist once. Unparseable input is dropped rather than rejected, so a stale bookmarked range degrades to the endpoint's original window instead of a 400. | — | `timesheet.controller.ts`, `report.controller.ts`, `dashboard.controller.ts` |

**Why `pdf-kit` is a module and not a convention.** Four services each carried a copy of the same
constants and helpers — `requirements-doc-pdf.service.ts` said so in its own header ("copied here
rather than imported, since that file's helpers are module-local") — and the copies had already
drifted: only one drew the watermark, only one had a running header, only one measured row heights
before drawing a table. A customer receiving a timesheet report and a requirements document from
the same product got two visibly different documents. Consolidating them also surfaced four latent
layout defects that only a rendered page shows, including multi-word status pills losing every word
after the first.
...
| File | Purpose | Depends on | Depended on by |
|---|---|---|---|
| `components/ui/calendar-primitives.tsx` | The one month-grid every picker renders: nav buttons, heading, day-cell styling with a deliberate state priority (selection beats today beats hover), a per-day hover card whose rows can carry a `separatorBefore` rule so counts of different KINDS of thing (log entries, tickets, changes) read as separate tallies rather than one total split three ways, and a fixed minimum grid height — months span 5 or 6 week-rows, and a popover that resizes while paging jumps under the cursor (and made WebKit's click-stability check time out). | `react-aria-components`, `@internationalized/date` | `date-picker.tsx`, `date-range-picker.tsx` |
| `components/ui/date-picker.tsx` | `DatePicker` (single date, commits on click, Today/Clear), `DateTimePicker` (calendar + a time-slot column that always includes the value it was handed — a picker that cannot express its own current value is broken by construction), and `TimeField` (segmented hh:mm AM/PM entry emitting 24-h `HH:mm`, for surfaces like the timesheet where any minute is legal and a slot grid would round 09:15 away). | `calendar-primitives.tsx` | Timesheet entry, ticket planning panel, maintenance window scheduling |
| `components/ui/date-range-picker.tsx` | `DateRangePicker` with nine presets (computed **at open time** — at module load, "today" freezes overnight), a two-month grid that collapses to one below `md`, and explicit Apply/Cancel. The trigger label derives only from the **committed** value, never the draft — deriving it from the draft is how Cancel leaves the trigger describing a range that was never applied (a bug this component's own spec caught on first run). `""` means unbounded, for "All time" on surfaces that allow it. | `calendar-primitives.tsx` | Reports, analytics, History, Workload, project planned-window + attestation period (admin), **and the home page's universal filter** — one range in the header drives every card on it, including the day timeline, which used to carry a second picker of its own |
...
| `workers/org-usage-snapshot.worker.ts` | Nightly at 03:40 UTC (the fifth offset, after trial lifecycle :00, backups :05, the database sampler :25 and retention :30 — four jobs opening tenant databases in the same second is a self-inflicted thundering herd). Skips rather than queues when a pass overruns, which is safe here because the capture upserts on `(organizationId, day)`. Daily rather than hourly because the questions it answers — churn, retention, cohort survival, revenue movement — are measured in months. | `platform-admin-analytics.service.ts` | `server.ts` |
| `services/platform-admin-analytics.service.ts` | The **single audited place** allowed to loop every tenant database for reporting, and therefore where the nightly capture lives too — a second such loop in a new file would make the isolation guarantee true of one file and false of the codebase. Every query in it is a count, a group-by count or a sum. `getPlatformAnalytics()` stays the live "as of right now" path; `captureOrgUsageSnapshots()` writes the history everything else reads. | `control-prisma.ts`, `prisma.ts`, `tenant-context.ts` | `platform-admin.controller.ts`, `platform-admin-console.controller.ts`, `org-usage-snapshot.worker.ts` |
| `services/platform-revenue.service.ts` | List MRR/ARR/ARPA, churn, NRR/GRR, trial→paid conversion, signup cohorts, seat overage and the fleet usage trend — **control-plane reads only**, no tenant connection, by construction. Every money figure carries `basis: "list-price"` because a list-price MRR is not billed revenue, and an unset price (Enterprise) is excluded and counted rather than summed as zero. Every ratio returns `null` rather than 0% when its denominator is empty — the normal state while the snapshot series is short, which cannot be backfilled. | `control-prisma.ts`, `platform-account-health.ts` | `platform-admin-console.controller.ts` |
| `services/platform-governance.service.ts` | The two-person queue. `twoPerson(action, execute)` produces the queueing route handler and registers the executor from **one closure**, and approval looks the executor up by *action* rather than by the stored route — so a countersigned action re-runs every guard against live data instead of re-deriving what to do from a serialised request. Refuses self-approval, non-pending rows and expired ones (24h), and a failed replay records `FAILED` without re-arming. | `control-prisma.ts`, `platform-audit.service.ts` | `platform-admin.controller.ts`, `platform-admin-console.controller.ts` |
| `utils/totp.ts` | RFC 6238 on `node:crypto` — no package, because a six-line HMAC is not worth a supply-chain edge and *is* worth pinning against the specification's own published vectors, which the test does. The step ratchet (`mfaLastUsedStep`) is here rather than at the call site so no caller can forget it. | `node:crypto` | `platform-admin-auth.service.ts` |
| `services/platform-alerts.service.ts` | Turns `deriveAlerts` (which has existed since 4.0.0 and only ever ran while somebody watched the page) into a **diff** against `PlatformAlertState`: appeared, escalated, cleared. A standing alert is never re-sent. `reportedSeverity` is written only when a channel accepted the message, so a failed send self-heals on the next pass instead of being recorded as delivered. | `control-prisma.ts`, `platform-tenant-health.service.ts` | `platform-alert-digest.worker.ts`, `platform-admin-console.controller.ts` |
| `utils/feature-overrides.ts` | The 17-key allowlist for per-org entitlement overrides, plus `classifyOverrides` — which separates a *grant* (past what the plan includes) from a *restriction*. That distinction is the whole feature: a restriction is an operator tightening their own product, a grant is one giving away a plan boundary, and only the second needs an acknowledgement and a name on the audit row. | `@timesheet/shared` | `plan-limits.service.ts`, `platform-feature-overrides.service.ts` |
| `services/platform-account-health.ts` | At risk / healthy / expansion **with the reason attached**. Pure — no database, no clock — so the rules can be interrogated one at a time; `signals` is never empty and `primarySignal` always names something measured, because a band nobody can explain is a number nobody acts on and therefore a number nobody maintains. Also owns the seat-pressure threshold the overage list uses, so the KPI tile and the list it links to cannot disagree. | `@timesheet/shared` | `platform-revenue.service.ts`, `platform-ai.service.ts` (as facts, not conclusions) |
