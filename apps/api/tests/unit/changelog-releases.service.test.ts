...
describe("parseChangelogReleases", () => {
  it("parses the repo's actual CHANGELOG.md into the full release history", () => {
    ...
    expect(releases[4].name).toBe("v1.1.0");
    expect(releases[4].publishedAt).toBe("2026-08-03T12:00:00.000Z");
    expect(releases[5].publishedAt).toBe("2026-07-29T12:00:00.000Z");

    // Bodies must not bleed across sections: 1.0.0's notes cannot mention the planning layer.
    expect(releases[5].notes).not.toContain("planning layer");
  });
