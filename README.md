...
| **Face (identity) verification (opt-in)** | Optional live camera check confirming the person submitting a timesheet or creating a ticket is the account holder — closing the "buddy punching" gap where one employee submits on a colleague's behalf. **All matching happens server-side** (the browser is a dumb camera; a client that decides its own verification outcome is not a security control), with anti-spoof + liveness scoring so a printed photo or phone screen can't defeat it. Off by default, with a workspace master switch plus per-user opt-in, an admin-tunable match threshold, and a review log of every attempt. Because this stores **biometric data** (GDPR Art.9 / Illinois BIPA / India DPDP), enrollment is consent-gated with the exact wording stored verbatim, templates are encrypted at rest, captured images are served only through an authenticated route and auto-purged on a retention schedule, and users can delete their own face data at any time. See [docs/FACE_VERIFICATION.md](docs/FACE_VERIFICATION.md). |
...
- **Testing**: Playwright (Chromium, custom phone/tablet/laptop/desktop/4K viewport projects) for end-to-end auth, tickets, timesheet, settings, and responsive-layout coverage; Vitest (`apps/api/tests`) for unit tests of the AI/billing/SCIM/face-verification services plus integration tests against a real throwaway MySQL database
...
npm run verify:face:e2e -w apps/api   # full HTTP flow against a running API (19 checks)
```
