...
| **Requirements Studio (opt-in)** | An AI-guided interview (Plan → Requirements Studio) that turns a project idea into a structured PRD/BRD before any project exists: one question at a time — problem, users, scope, features, tech stack, architecture, timeline, risks, success metrics — with quick-reply suggestions and a "skip — assume and flag it" option so the interview never stalls, and anything left uncovered becomes a stated **assumption** in the finished document rather than a silent guess. Generates a real navigable section-by-section document, including an AI-drawn architecture diagram (rendered live via Mermaid, falling back to raw source rather than the page if a weaker model draws something unparseable), and exports to PDF and Markdown. Turning it into real work stays human-reviewed at every step: "Create project from this document" prefills the New Project form, "Propose tickets" turns every feature into a draft change set accepted row-by-row through the same trusted pipeline `plan_breakdown` already uses, and "Create goals" turns success metrics into goals after a one-screen confirm — nothing is ever created silently. |
